#!/usr/bin/env python3
"""
Scan City Parks Association Photographs for train/railroad content.
Compares metadata keyword matches vs CLIP visual detection.
Outputs results JSON and a comparison summary.
"""
...
OUTPUT_PATH = Path("image-analysis/cityparks_train_results.json")


def get_all_items():
    """Fetch all items with title, description, subjects via dmQuery."""
    url = (
...
def main():
    ...
    print("Fetching City Parks item list...", flush=True)
    items = get_all_items()
    print(f"Found {len(items)} items.\n", flush=True)

    # Phase 1: identify metadata matches
    ...
    # Phase 2: CLIP scan of everything
    clip_matches = {}
    ...
    # Build output items for all matches
